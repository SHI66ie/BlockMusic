...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, mode, onSwitchMode }) => {
  ...
  useEffect(() => {
    const handleGoogleSuccess = (event: CustomEvent) => {
      ...
      alert(`Google ${mode} successful! Welcome, ${user.name}`);
      onClose();
    };
      ...
      <div className="bg-gray-900 rounded-2xl p-8 w-full max-w-md relative border border-gray-700">
        ...
        <div className="mb-8 space-y-4">
          <button
            ...
            className="w-full bg-neutral-800 hover:bg-neutral-700 text-neutral-100 py-3 rounded-xl font-medium transition-all duration-200 flex items-center justify-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed border border-neutral-600"
          >
            {isGoogleLoading ? (
